...
app.post('/api/users/:uid/grant-premium', async (req, res) => {
  try {
    ...
    await db.collection('users').doc(req.params.uid).update({
      ...
      grantedByAdminUid: (req.adminUid || 'admin'),
    });
    res.json({ success: true, expiresAt: expires.toISOString() });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/users/:uid/revoke-premium', async (req, res) => {
  try {
    await db.collection('users').doc(req.params.uid).update({
      plan:              'free',
      ...
      revokedByAdminUid: (req.adminUid || 'admin'),
    });
...
app.delete('/api/qrcodes/:code', async (req, res) => {
  try {
...
//  BIO PAGES
// ═══════════════════════════════════════════════════════════════════════════════
app.get('/api/biopages', async (req, res) => {
  try {
...
app.delete('/api/biopages/:username', async (req, res) => {
  try {
...
app.post('/api/broadcasts', async (req, res) => {
  try {
    ...
    const ref = await db.collection('broadcasts').add({
      ...
      target,
      sentByUid:   (req.adminUid || 'admin'),
      sentByEmail: (req.adminEmail || 'admin'),
